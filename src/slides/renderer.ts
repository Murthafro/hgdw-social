...
/**
 * Renders deck slides to 1080×1920 PNGs (9:16 vertical). Supports two modes:
 *   launch — Playwright launches its own Chromium
 *   cdp    — attach to an already-running Chrome over CDP
 */
export class SlideRenderer {
  ...
  constructor(opts: RendererOptions = {}) {
    this.opts = {
      mode: opts.mode ?? (process.env.HGDW_RENDERER as RendererMode) ?? "launch",
      cdpUrl: opts.cdpUrl ?? process.env.HGDW_CDP_URL ?? "http://localhost:29229",
      chromePath: opts.chromePath ?? process.env.HGDW_CHROME_PATH ?? "",
    };
  }
  ...
  async renderDeck(
    ...
    onRendered?: (frameId: string, pngPath: string) => void,
  ): Promise<string[]> {
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      written.push(out);
      if (onRendered) onRendered(frame.frameId, out);
    }
    await page.close();
    return written;
  }
